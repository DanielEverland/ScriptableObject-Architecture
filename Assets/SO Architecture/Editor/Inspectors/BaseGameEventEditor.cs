...
{
    private GameEventStackTraceBase StackTraceBase { get { return (GameEventStackTraceBase)target; } }
    ...
    private void DrawList()
    {
        ...
        {
            height = StackTraceBase.StackTraces.Count * LINE_HEIGHT,
            width = scrollViewRect.width - 20,
        };

        _listScrollPosition = GUI.BeginScrollView(scrollViewRect, _listScrollPosition, position);

        for (int i = 0; i < StackTraceBase.StackTraces.Count; i++)
        {
            GameEventStackTrace currentTrace = StackTraceBase.StackTraces[i];
    ...
    private void DrawStackTraceHeader()
    {
        ...
        {
            StackTraceBase.StackTraces.Clear();
            Deselect();
        }
    }
